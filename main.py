...
df = read_tntp_network("Austin_net.tntp")
...
if '6665' in graph:
    print(" node 6665 is in the graph ")
...
y_init, candidates = initialize_y_with_random_removals(links, fraction=0.1, seed=2)


print("Randomly removed links (y=0):", candidates)
...
demand = read_trips_tntp("Austin_trips_am.tntp")
...
def daul_value(graph, origin, destination, y_building , links):
    ...
    gamma = demand[origin][destination] * 1 *  find_f_od(graph, origin, destination, c, y_building, od_costs) # sum(h_pi.values()) * find_f_od(graph, origin, destination, c, y_building, od_costs)  # Sum of h pi values
...
while gap > gap_threshold:
    ...
    od_costs = {}
    for node in nodes:
        od_costs[node] = find_cost_od(graph, nodes, node, y_init)
    ...
    for od in od_pairs:
        ...
        objvalue_subproblem +=  1 * find_f_od(graph, orig, dest, c, y_init, od_costs) * demand[orig][dest] #* o[orig] * w[dest]
